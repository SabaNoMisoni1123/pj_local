...
- `.agents/prompts/`: 命令セットを点検・更新するための保守用プロンプト。
- `.agents/reports/`: 監査結果、再編提案、未対応事項の記録。
...
- description が似て発火競合するスキルは、境界を明確にする。大規模統合は先に `reports/` に提案を書く。
- 使われていないスキルでも、削除前に代替スキル、影響範囲、移行先をレポート化する。
...
7. 監査や大きな再編は `.agents/reports/` に記録する。
